...
const ImageLoadingSpinner: React.FC = () => (
    <div className="flex items-center justify-center h-48 bg-black/50 rounded-lg mb-4">
        <div className="w-10 h-10 border-4 border-yellow-400 border-dashed rounded-full animate-spin border-t-transparent"></div>
    </div>
);
...
const DiceRollDisplay: React.FC<{ roll: DiceRoll }> = ({ roll }) => {
    ...
    const modifierText = roll.modifier >= 0 ? `+ ${roll.modifier}` : `- ${Math.abs(roll.modifier)}`;

    return (
        <div className="bg-gray-900/50 border border-gray-600 rounded-lg p-3 my-4 text-center">
            <p className="font-bold text-yellow-400 text-lg cinzel">{t('skillCheck', { skill: roll.skill })}</p>
        ...
        {safeStoryLog.map((entry) => {
           switch (entry.speaker) {
            case 'story':
              return (
                <div key={entry.id}>
                    {entry.imageIsLoading && <ImageLoadingSpinner />}
                    {entry.imageUrl && (
                        <img 
                            src={entry.imageUrl} 
                            alt="A depiction of the story scene." 
                            className="rounded-lg mb-4 w-full object-cover shadow-lg border-2 border-gray-600"
                        />
                    )}
            ...
            case 'system':
              return <p key={entry.id} className="text-center text-yellow-400 text-sm italic">{entry.text}</p>;
