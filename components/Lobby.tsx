        ...
        return (
             <div className="min-h-screen w-screen bg-gray-900 text-gray-200 flex flex-col items-center justify-center p-4">
                <div className="bg-gray-800 p-8 rounded-lg shadow-2xl max-w-2xl w-full text-center">
                    <h1 className="text-4xl font-bold text-yellow-400 mb-4 cinzel">{t('partyLobby')}</h1>
                    ...
                    >
                     ...
                     <h2 className="text-2xl font-bold text-yellow-400 mb-4 cinzel">{t('adventurersJoined')}</h2>
                        ...
                        {gameState.players.length > 0 ? gameState.players.map(player => (
                            <div key={player.id} className="bg-gray-700 p-3 rounded-lg text-center">
                                <div className="w-16 h-16 mx-auto mb-2 rounded-full bg-gray-900 border-2 border-yellow-400 flex items-center justify-center overflow-hidden shadow-lg">
                                     {player.avatarIsLoading && <div className="w-6 h-6 border-2 border-gray-500 border-dashed rounded-full animate-spin border-t-transparent"></div>}
                                    {!player.avatarIsLoading && player.avatarUrl && <img src={player.avatarUrl} alt={`Avatar of ${player.name}`} className="w-full h-full object-cover" />}
                                    {!player.avatarIsLoading && !player.avatarUrl && <span className="text-2xl text-gray-600">?</span>}
                                </div>
                                <p className="font-bold text-yellow-400">{player.name}</p>
                                <p className="text-xs text-gray-400">{`${player.race} ${player.background}`}</p>
                            </div>
                        )) : (
                            <p className="text-gray-500 italic col-span-full">{t('waitingForAdventurers')}</p>
                        )}
                    </div>
                    ...
                    {!isHost && <p className="text-yellow-400 italic">{t('waitingForHost')}</p>}
    ...
    return (
        <div className="min-h-screen w-screen bg-gray-900 text-gray-200 flex flex-col items-center justify-center p-4">
            <div className="bg-gray-800 p-8 rounded-lg shadow-2xl max-w-md w-full">
                <h1 className="text-4xl font-bold text-yellow-400 mb-6 cinzel text-center">{t('joinAdventure')}</h1>
                ...
                <div className="space-y-6">
                    ...
                    <form onSubmit={handleJoinSubmit} className="space-y-3">
                         <label htmlFor="joinId" className="block text-lg font-semibold cinzel text-gray-300 text-center">{t('joinExistingGame')}</label>
                        <input
                            type="text" id="joinId" value={joinId} onChange={(e) => setJoinId(e.target.value)}
                            ...
                            placeholder={t('enterGameId')} required
                        />
                         <button
                            type="submit"
